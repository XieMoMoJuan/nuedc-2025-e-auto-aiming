...
void OLED_W_SCL(uint16_t BitValue)
{
	/*根据BitValue的值，将SCL置高电平或者低电平*/
	//GPIO_WriteBit(GPIOB, GPIO_Pin_8, (BitAction)BitValue);
	//BitValue==0 ? DL_GPIO_clearPins(OLEDS_PORT, OLEDS_SCL_PIN) : DL_GPIO_setPins(OLEDS_PORT, OLEDS_SCL_PIN);delay_cycles(320);
 ...
 */
void OLED_W_SDA(uint16_t BitValue)
{
	/*根据BitValue的值，将SDA置高电平或者低电平*/
	//BitValue==0 ? DL_GPIO_clearPins(OLEDS_PORT, OLEDS_SDA_PIN) : DL_GPIO_setPins(OLEDS_PORT, OLEDS_SDA_PIN);delay_cycles(320);
...
void OLED_I2C_SendByte(uint8_t Byte)
{
    ...
    DL_I2C_fillControllerTXFIFO(I2C_1_OLED_INST, &Byte, 1);
    while(! (DL_I2C_getControllerStatus(I2C_1_OLED_INST) & DL_I2C_CONTROLLER_STATUS_IDLE));
    DL_I2C_startControllerTransfer(I2C_1_OLED_INST, 0x3C, DL_I2C_CONTROLLER_DIRECTION_TX, 1);
    while(DL_I2C_getControllerStatus(I2C_1_OLED_INST) & DL_I2C_CONTROLLER_STATUS_BUSY);
    while(! (DL_I2C_getControllerStatus(I2C_1_OLED_INST) & DL_I2C_CONTROLLER_STATUS_IDLE));
    DL_I2C_flushControllerTXFIFO(I2C_1_OLED_INST);
}
...
void OLED_WriteCommand(uint8_t Command)
{
    ...
    DL_I2C_fillControllerTXFIFO(I2C_1_OLED_INST, data, 2);
    while(! (DL_I2C_getControllerStatus(I2C_1_OLED_INST) & DL_I2C_CONTROLLER_STATUS_IDLE));
    DL_I2C_startControllerTransfer(I2C_1_OLED_INST, 0x3C, DL_I2C_CONTROLLER_DIRECTION_TX, 2);
    while(DL_I2C_getControllerStatus(I2C_1_OLED_INST) & DL_I2C_CONTROLLER_STATUS_BUSY);
    while(! (DL_I2C_getControllerStatus(I2C_1_OLED_INST) & DL_I2C_CONTROLLER_STATUS_IDLE));
    DL_I2C_flushControllerTXFIFO(I2C_1_OLED_INST);
}
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
    ...
    // for (i = 0; i < Count; i++)
    // {
    //     OLED_I2C_SendByte(Data[i]); // 依次发送Data的每一个数据s
    // }
    // OLED_I2C_Stop(); // I2C终止

    uint8_t chunk, sent = 0;
    uint8_t buf[8]; // 1字节控制+7字节数据

    while (sent < Count) {
        chunk = (Count - sent > 7) ? 7 : (Count - sent);
        buf[0] = 0x40; // 控制字节
        memcpy(&buf[1], &Data[sent], chunk);

        DL_I2C_fillControllerTXFIFO(I2C_1_OLED_INST, buf, chunk + 1);
        while (!(DL_I2C_getControllerStatus(I2C_1_OLED_INST) & DL_I2C_CONTROLLER_STATUS_IDLE));
        DL_I2C_startControllerTransfer(I2C_1_OLED_INST, 0x3C, DL_I2C_CONTROLLER_DIRECTION_TX, chunk + 1);
        while (DL_I2C_getControllerStatus(I2C_1_OLED_INST) & DL_I2C_CONTROLLER_STATUS_BUSY);
        while (!(DL_I2C_getControllerStatus(I2C_1_OLED_INST) & DL_I2C_CONTROLLER_STATUS_IDLE));
        DL_I2C_flushControllerTXFIFO(I2C_1_OLED_INST);
