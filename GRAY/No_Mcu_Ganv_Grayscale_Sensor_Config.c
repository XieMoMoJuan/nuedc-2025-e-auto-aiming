...
unsigned short white[8] = {2792, 2644, 1647, 2477, 2582, 1380, 1126, 1090};
unsigned short black[8] = {95, 92, 89, 95, 142, 93, 95, 67};
...
unsigned int adc_getValue(void)
{
    ...
    // 软件触发ADC开始转换
    DL_ADC12_startConversion(ADC_12_0_GRY_INST);

    // 如果当前状态 不是 空闲状态
    while (DL_ADC12_getStatus(ADC_12_0_GRY_INST) != DL_ADC12_STATUS_CONVERSION_IDLE)
        ;

    // 清除触发转换状态
    DL_ADC12_stopConversion(ADC_12_0_GRY_INST);
    // 失能ADC转换
    DL_ADC12_disableConversions(ADC_12_0_GRY_INST);

    // 获取数据
    gAdcResult = DL_ADC12_getMemResult(ADC_12_0_GRY_INST, ADC_12_0_GRY_ADCMEM_0);
...
void No_MCU_Ganv_Sensor_Init(No_MCU_Sensor *sensor)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        // 计算灰度阈值（1:2和2:1分界点）
        sensor->Gray_white[i] = (white[i] * 2 + black[i]) / 3;
        sensor->Gray_black[i] = (white[i] + black[i] * 2) / 3;

        // 保存校准数据
        sensor->Calibrated_black[i] = black[i];
        sensor->Calibrated_white[i] = white[i];
        ...
        // 计算归一化系数
        Normal_Diff[i] = (double)white[i] - (double)black[i];
...
void set_gray(void)
{
    ...
    {
        OLED_ShowString(32, 0, "SET_WHITE", OLED_8X16);
        OLED_ShowString(32, 32, "PRESS_KEY0", OLED_8X16);
        Get_Analog_value(result);
        for (int i = 0; i < 8; i++)
        {
            OLED_ShowNum(0, 8 * i, result[i], 4, OLED_6X8);
        }
        OLED_Update();
        delay_ms(10);
    }
    memcpy(white, result, 8);
    OLED_Clear();
    OLED_Update();
    while (DL_GPIO_readPins(KEYS_PORT, KEYS_KEY1_PIN) != KEYS_KEY1_PIN)
    {
        OLED_ShowString(32, 0, "SET_BLACK", OLED_8X16);
        OLED_ShowString(32, 32, "PRESS_KEY1", OLED_8X16);
        Get_Analog_value(result);
        for (int i = 0; i < 8; i++)
        {
            OLED_ShowNum(0, 8 * i, result[i], 4, OLED_6X8);
